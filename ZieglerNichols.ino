...
void runPositionControl() {
  ...
  double pos_Kp = (double)Kp_potValue / 1023.0 * 2.25;

  int Ki_potValue = analogRead(POT_KI_PIN);
  double pos_Ki = (double)Ki_potValue / 1023.0 * 0.15;

  int Kd_potValue = analogRead(POT_KD_PIN);
  double pos_Kd = (double)Kd_potValue / 1023.0 * 0.3;
...
void runSpeedControl() {
  ...
  // --- CORRECTED GAIN MAPPING ---
  int Kp_potValue = analogRead(POT_KP_PIN);
  double spd_Kp = (double)Kp_potValue / 1023.0 * 3.0;

  int Ki_potValue = analogRead(POT_KI_PIN);
  double spd_Ki = (double)Ki_potValue / 1023.0 * 1.5;
